define pacemaker::resource::base ($resource_type,
  ...
  if($ensure == absent) {
    ...
    #  require => [Exec["Start Cluster ${corosync::cluster_name}"],Package["corosync"]]
    #}
    exec { "Creating ${name}":
      command => "/usr/sbin/crm configure primitive ${name} ${resource_type} params ${resource_params} op monitor interval=${interval}${group_option}",

      unless  => "/usr/sbin/crm resource show ${name} > /dev/null 2>&1",
      require => [Exec["Start Cluster ${corosync::cluster_name}"],Package["corosync"]]
    }
